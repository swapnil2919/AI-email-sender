...
@keyframes orb1 {
  0%,100%{transform:translate(0,0) scale(1)} 33%{transform:translate(70px,-55px) scale(1.1)}
  66%{transform:translate(-40px,45px) scale(.92)}
}
@keyframes orb2 {
  0%,100%{transform:translate(0,0) scale(1)} 40%{transform:translate(-80px,60px) scale(1.08)}
...
@keyframes borderFlow {
  0%{border-color:rgba(20,184,166,.35)} 33%{border-color:rgba(59,130,246,.5)}
  66%{border-color:rgba(168,85,247,.4)} 100%{border-color:rgba(20,184,166,.35)}
}
@keyframes dot { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.3;transform:scale(.6)} }
@keyframes float { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-10px)} }
@keyframes cardIn {
  from{opacity:0;transform:translateY(24px) scale(.98)} to{opacity:1;transform:translateY(0) scale(1)}
}

.stApp {
  background:#04060f; min-height:100vh; overflow-x:hidden;
}
.stApp::before {
  content:''; position:fixed; z-index:0; pointer-events:none; border-radius:50%; filter:blur(120px);
  width:700px; height:700px;
  background:radial-gradient(circle,rgba(20,184,166,.13) 0%,transparent 70%);
  top:-160px; right:-130px; animation:orb1 18s ease-in-out infinite;
}
.stApp::after {
  content:''; position:fixed; z-index:0; pointer-events:none; border-radius:50%; filter:blur(130px);
  width:750px; height:750px;
  background:radial-gradient(circle,rgba(99,102,241,.11) 0%,transparent 70%);
  bottom:-190px; left:-150px; animation:orb2 22s ease-in-out infinite;
}
...
.stTextInput input:focus,.stTextArea textarea:focus {
  ...
  background:rgba(20,184,166,.07) !important; transform:translateY(-1px) !important;
}
...
.stNumberInput label,.stFileUploader label {
  color:#5eead4 !important; font-size:10px !important; font-weight:800 !important;
  text-transform:uppercase !important; letter-spacing:1.1px !important;
}
[data-baseweb="select"] > div {
  background:rgba(255,255,255,.05) !important;
  border:1.5px solid rgba(255,255,255,.1) !important;
  border-radius:14px !important; color:#f1f5f9 !important; transition:all .3s !important;
}
[data-baseweb="select"] > div:focus-within {
  border-color:#14b8a6 !important; box-shadow:0 0 0 3px rgba(20,184,166,.15) !important;
}
...
.stButton>button[kind="secondary"]:hover {
  ...
  transform:translateY(-2px) !important; box-shadow:0 6px 24px rgba(20,184,166,.3) !important;
}
...
.slabel::after { content:''; flex:1; height:1px; background:linear-gradient(90deg,rgba(20,184,166,.3),transparent); }

.badge {
  display:inline-flex; align-items:center; gap:6px;
  padding:5px 13px; border-radius:20px; font-size:11px; font-weight:700;
  margin:3px 0; transition:all .2s;
}
...
.dot { width:7px; height:7px; border-radius:50%; display:inline-block; animation:dot 2s ease-in-out infinite; }
.bg .dot{background:#6ee7b7} .br .dot{background:#fca5a5}
.bt .dot{background:#99f6e4} .bb .dot{background:#93c5fd}
...
.hint {
  ...
  padding:12px 16px; font-size:13px; color:#99f6e4; margin:8px 0 4px; line-height:1.6;
  animation:slideDown .4s ease both; transition:all .3s;
}
.hint:hover { border-color:rgba(20,184,166,.5); }
...
.arow {
  ...
  border-radius:12px; padding:9px 16px; margin:5px 0; font-size:13px; color:#e2fff9;
...
/* ── Config page ── */
.cfg-hero {
  text-align:center; padding:50px 20px 36px; animation:fadeDown .7s ease both;
}
.cfg-logo {
  width:80px; height:80px; margin:0 auto 20px;
  background:linear-gradient(135deg,#14b8a6,#0ea5e9); border-radius:24px;
  display:flex; align-items:center; justify-content:center; font-size:38px;
  box-shadow:0 8px 40px rgba(20,184,166,.5); animation:float 4s ease-in-out infinite;
...
.cfg-card {
  background:rgba(255,255,255,.04); border-radius:24px; padding:32px;
  backdrop-filter:blur(20px); position:relative; overflow:hidden;
  animation:cardIn .6s ease both; transition:border-color .3s, box-shadow .3s;
  margin-bottom:20px;
}
...
.cfg-card-ai::before   { background:linear-gradient(90deg,#14b8a6,#38bdf8); border:1px solid rgba(20,184,166,.15); }
.cfg-card-smtp::before { background:linear-gradient(90deg,#8b5cf6,#ec4899); border:1px solid rgba(139,92,246,.15); }
.cfg-card-ai   { border:1px solid rgba(20,184,166,.15); }
.cfg-card-smtp { border:1px solid rgba(139,92,246,.15); }
.cfg-card::after {
...
.step-header { display:flex; align-items:center; margin-bottom:22px; gap:14px; }
.step-num {
...
.step-num-ai   { background:linear-gradient(135deg,#14b8a6,#0ea5e9); box-shadow:0 4px 16px rgba(20,184,166,.4); }
.step-num-smtp { background:linear-gradient(135deg,#8b5cf6,#ec4899); box-shadow:0 4px 16px rgba(139,92,246,.4); }
.step-info h3 { margin:0; font-size:18px; font-weight:800; color:#f0fdfa; }
.step-info p  { margin:2px 0 0; font-size:12px; color:rgba(255,255,255,.38); }
...
.status-row .si { font-size:22px; flex-shrink:0; }
.status-row .st h4 { margin:0; font-size:14px; font-weight:700; color:#e2e8f0; }
.status-row .st p  { margin:0; font-size:12px; color:rgba(255,255,255,.32); }
.status-row .sb { margin-left:auto; }

/* ── Compose page ── */
.topbar { display:flex; align-items:center; padding:16px 0 22px; animation:fadeDown .5s ease both; }
.topbar-icon {
  width:42px; height:42px; background:linear-gradient(135deg,#14b8a6,#0ea5e9);
  border-radius:13px; font-size:20px; display:flex; align-items:center; justify-content:center;
  box-shadow:0 4px 18px rgba(20,184,166,.4); animation:breatheTeal 3s ease-in-out infinite;
  margin-right:12px; flex-shrink:0;
}
.topbar h2 { margin:0; font-size:20px; font-weight:900; color:#f0fdfa; }
.topbar p  { margin:0; font-size:11px; color:rgba(94,234,212,.5); font-weight:700; letter-spacing:1px; text-transform:uppercase; }

.card {
  background:rgba(255,255,255,.035); border-radius:22px; padding:24px 28px; margin-bottom:16px;
  backdrop-filter:blur(16px); position:relative; overflow:hidden;
  transition:border-color .3s, box-shadow .35s, transform .3s; animation:cardIn .55s ease both;
}
...
#  SESSION DEFAULTS
# ══════════════════════════════════════════════════════════════════════════════
for k, v in {"page": "config", "api_key": "", "model_id": None,
             "smtp_host": "smtp.gmail.com", "smtp_port": 587,
             "smtp_user": "", "smtp_pass": "", "body": ""}.items():
...
if st.session_state.page == "config":
    ...
    with mid:
        ...
        <div class="step-header">
          <div class="step-num step-num-ai">1</div>
          <div class="step-info">
            <h3>AI Engine</h3>
        ...
        with c1:
            fetch_btn = st.button("⚡ Load All Models", use_container_width=True,
                                  disabled=not api_key, key="fetch_btn")
        with c2:
            if "models" in st.session_state and st.session_state.models:
                st.markdown(
                    f'<div class="badge bt"><span class="dot"></span>&nbsp;'
                    f'{len(st.session_state.models)} models</div>',
                    unsafe_allow_html=True)
        ...
        if st.session_state.get("models"):
            ...
            chosen  = st.selectbox("Select AI Model", lbls, index=di, key="model_select")
            model_id = ids[lbls.index(chosen)]
            st.markdown(f'<div class="mchip">📌 {model_id}</div>', unsafe_allow_html=True)
        elif not api_key:
            st.markdown(
                '<div class="hint">🔑 Paste your OpenRouter API key above, then click <b>Load All Models</b>.<br>'
                'Get a free key at <b>openrouter.ai/keys</b></div>',
                unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="step-header">
          <div class="step-num step-num-smtp">2</div>
          <div class="step-info">
            <h3>Email Account</h3>
        ...
        with sc1:
            smtp_host = st.text_input("SMTP Host", value=st.session_state.smtp_host,
                                      placeholder="smtp.gmail.com", key="inp_host")
        with sc2:
            smtp_port = st.number_input("Port", value=int(st.session_state.smtp_port),
                                        min_value=1, max_value=65535, step=1, key="inp_port")
        su1, su2 = st.columns(2)
        with su1:
            smtp_user = st.text_input("Your Email", value=st.session_state.smtp_user,
                                      placeholder="you@gmail.com", key="inp_user")
        with su2:
            smtp_pass = st.text_input("App Password", type="password",
                                      value=st.session_state.smtp_pass,
                                      placeholder="xxxx xxxx xxxx xxxx", key="inp_pass")

        st.markdown("""
        <div class="hint" style="margin-top:12px">
        💡 &nbsp;For Gmail — generate an <b>App Password</b> at
        <b>myaccount.google.com → Security → App passwords</b>.
        Use that instead of your main password.
        ...
        <div class="status-row">
          <span class="si">{"✅" if ai_ok else "❌"}</span>
          <div class="st">
            <h4>AI Engine</h4>
            <p>{"Model: " + (model_id or "") if ai_ok else "Add API key and load models"}</p>
          </div>
          <div class="sb">
            <div class="badge {"bg" if ai_ok else "br"}">
              <span class="dot"></span>&nbsp;{"Ready" if ai_ok else "Missing"}
            </div>
          </div>
        </div>
        <div class="status-row">
          <span class="si">{"✅" if smtp_ok else "❌"}</span>
          <div class="st">
            <h4>Email Account</h4>
            <p>{"Sending from: " + smtp_user if smtp_ok else "Enter email and app password"}</p>
          </div>
          <div class="sb">
            <div class="badge {"bg" if smtp_ok else "br"}">
              <span class="dot"></span>&nbsp;{"Ready" if smtp_ok else "Missing"}
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("🚀  Start Composing →", use_container_width=True,
                     type="primary", disabled=not (ai_ok and smtp_ok)):
        ...
        if not (ai_ok and smtp_ok):
            missing = []
            if not ai_ok:   missing.append("AI setup")
            if not smtp_ok: missing.append("SMTP credentials")
            st.markdown(
                f'<div class="hint hint-warn" style="text-align:center">'
                f'⚠️ &nbsp;Complete <b>{" & ".join(missing)}</b> to continue.</div>',
                unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    smtp_cfg = {k: st.session_state[f"smtp_{k}"] for k in ["host", "port", "user", "pass"]}

    # Topbar
    col_logo, _, col_btn = st.columns([4, 4, 2])
    with col_logo:
        st.markdown("""
        <div class="topbar">
    ...
    st.markdown(
        f'<div class="badge bt" style="margin-bottom:18px">'
        f'<span class="dot"></span>&nbsp;{model_id.split("/")[-1]}</div>',
        unsafe_allow_html=True)
    ...
    with cb:
        gen_btn = st.button("✨ Generate Body", use_container_width=True, disabled=not subject)
    with ch:
        if not subject:
            ...
            st.markdown(
                f'<div class="hint">✨ &nbsp;<b>{tone}</b> tone · <b>{model_id.split("/")[-1]}</b> — ready!</div>',
                unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    body = st.text_area("Body", value=st.session_state.get("body", ""),
                        ...
                        placeholder="Click ✨ Generate Body to auto-fill, or type your message here…")
    ...
    if files:
        for f in files:
            st.markdown(
                f'<div class="arow">📄&nbsp;<b>{f.name}</b>'
                f'<span style="margin-left:auto;opacity:.4">{f.size/1024:.1f} KB</span></div>',
                unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if not ready:
        ...
        if missing:
            st.markdown(
                f'<div class="hint hint-warn">⚠️ &nbsp;Still needed: <b>{", ".join(missing)}</b></div>',
                unsafe_allow_html=True)
    ...
    if send and ready:
        with st.spinner("Sending your email…"):
            try:
                ...
                st.error("❌ Authentication failed — go to ⚙️ Settings and check your credentials.")
